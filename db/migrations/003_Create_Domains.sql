...
-------------------------------------------------------------------------------
CREATE DOMAIN public.uint8 AS SMALLINT CHECK (
  VALUE >= 0
  AND VALUE <= 255
);
...
-------------------------------------------------------------------------------
CREATE DOMAIN public.hexstring AS VARCHAR(255) CHECK (VALUE ~ '^0x([a-f0-9]{2})+$');
...
-------------------------------------------------------------------------------
CREATE DOMAIN public.eth_address AS VARCHAR(42) CHECK (VALUE ~ '^0x[a-f0-9]{40}$');
...
-------------------------------------------------------------------------------
CREATE DOMAIN public.eth_transaction_hash AS VARCHAR(66) CHECK (VALUE ~ '^0x[a-f0-9]{64}$');
